...
function JobForkLift:onBoxLoad(box)
	if isElement(box) and table.find(self.m_Boxes, box) then
		...
		local income = self:calculateJobLevelPay(client, MONEY_PER_BOX)
		self.m_BankAccount:transferMoney({client, true}, income, "Gabelstapler-Job", "Job", "ForkLift")
		if chance(50) then
			points = math.floor(1*JOB_EXTRA_POINT_FACTOR)
			client:givePoints(points)
		end
		StatisticsLogger:getSingleton():addJobLog(client, "jobForkLift", duration, income, nil, nil, points)
	end
end
